...
if not fromBam:
    ...
    rule rm_dupes:
        ...
        threads: nthreads
        conda: CONDA_SHARED_ENV
        shell: "tmp_dupes=$(mktemp -d -p {{$TMPDIR}} -t XXXXX.{wildcards.sample}); sambamba markdup --remove-duplicates -t {threads} --tmpdir {{$tmp_dupes}} {input.sbam} {output.rmDupbam} 1>{log.out} 2>{log.err}"
